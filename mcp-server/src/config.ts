...
export class ConfigLoader {
   ...
   */
  getProjects(): Array<{ id: string; config: ProjectConfig }> {
...
export class DataLoader {
   ...
   */
  private getDocSyncPath(projectId: string): string | null {
  ...
  loadNode(projectId: string, nodeId: string): NodeData | null {
    ...
    if (fs.existsSync(nodePath)) {
      try {
        ...
        return JSON.parse(content) as NodeData;
      } catch (error) {
  ...
  searchCodebase(projectId: string, query: string): Array<{
    ...
    for (const node of graph.nodes) {
      ...
      if (nodeDoc) {
        ...
        for (const term of queryTerms) {
          if (aiSummary.includes(term) || aiDescription.includes(term)) score += 1;
        }
      }
