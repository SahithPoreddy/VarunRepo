 ...
 */
function formatNode(node, includeCode = false) {
    const result = {
        id: node.id,
        label: node.label,
        type: node.type,
    ...
    if (node.metadata) {
        if (node.metadata.startLine)
            result.startLine = node.metadata.startLine;
        if (node.metadata.endLine)
            result.endLine = node.metadata.endLine;
        if (node.metadata.parameters)
            result.parameters = node.metadata.parameters;
        if (node.metadata.returnType)
            result.returnType = node.metadata.returnType;
        if (node.metadata.docstring)
            result.docstring = node.metadata.docstring;
        if (node.metadata.aiSummary)
            result.aiSummary = node.metadata.aiSummary;
        if (node.metadata.aiDescription)
            result.aiDescription = node.metadata.aiDescription;
        if (node.metadata.patterns)
            result.patterns = node.metadata.patterns;
        if (node.metadata.keywords)
            result.keywords = node.metadata.keywords;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'get_node': {
                const { project, nodeId, includeCode = false } = args;
                if (!configLoader.getProject(project)) {
                    throw new McpError(ErrorCode.InvalidParams, `Project "${project}" not found`);
                }
            ...
            case 'add_project': {
                const { projectId, path, name, description } = args;
